...
func (*ElasticOutEaseProvider) New(params []float64) func(float64) float64 {
	...
	elastic := func(v float64) float64 {
		if math.Abs(0-v) < phomath.Epsilon {
			return 0
		...
		if amplitude < 1 {
			...
			s = period * math.Asin(1/amplitude) / phomath.PI2
		}

		return amplitude*math.Pow(2, -10*v)*math.Sin((v-s)*phomath.PI2/period) + 1
	}
...
func (*ElasticInEaseProvider) New(params []float64) func(float64) float64 {
	...
	elastic := func(v float64) float64 {
		...
		return -(amplitude * math.Pow(2, 10*(v-1)) * math.Sin((v-s)*phomath.PI2/period))
	}
...
func (*ElasticInOutEaseProvider) New(params []float64) func(float64) float64 {
	...
	elastic := func(v float64) float64 {
		...
		if amp < 1 {
			...
			s = period * math.Asin(1/amp) / phomath.PI2
		}

		v *= 2
		if v < 1 {
			return -0.5 * (amp * math.Pow(2, 10*(v-1)) * math.Sin((v-s)*phomath.PI2/period))
		}

		return amp*math.Pow(2, -10*(v-1))*math.Sin((v-s)*phomath.PI2/period)*0.5 + 1
	}
